...
EPUBJS.Reader = function(bookPath, _options) {
	...
	reader.DownloadController = EPUBJS.reader.DownloadController.call(reader, book);
	reader.ShareController = EPUBJS.reader.ShareController.call(reader, book);
	reader.HomeController = EPUBJS.reader.HomeController.call(reader, book);
...
EPUBJS.reader.ControlsController = function(book) {
	...
	$bookmarkempty.on("click", function() {
		var cfi = reader.book.getCurrentLocationCfi();
		var bookmarked = reader.isBookmarked(cfi);

		if(bookmarked === -1) { //-- Add bookmark
			reader.addBookmark(cfi);
			$bookmarkempty.addClass("hide");
			$bookmarkempty.removeClass("show");
			$bookmarkfull.removeClass("hide");
			$bookmarkfull.addClass("show");

		}

	});

	$bookmarkfull.on("click", function() {

        var cfi = reader.book.getCurrentLocationCfi();

		reader.removeBookmark(cfi);
		$bookmarkfull.addClass("hide");
		$bookmarkfull.removeClass("show");
		$bookmarkempty.addClass("show");
		$bookmarkempty.removeClass("hide");
	...
	var arrowKeys = function(e) {
		...
		if(e.keyCode == 37) { 
			...
			if(book.metadata.direction === "rtl") {
				//book.nextPage();
				//book.spinePos++;
				book.nextChapter(defer);
			} else {
				//book.prevPage();
                //book.spinePos--;
				book.prevChapter(defer);
			}
...
EPUBJS.reader.HomeController = function() {

	var $home = $("#home");
	var book = this.book;

	$home.on('click', function(e) {
		//book.renderer.gotoCfi()
		book.gotoCfi(0)
	});

};

EPUBJS.reader.DownloadController = function(download) {
	var $thing = $("#downloadView");
	var onShow = function() {
		$thing.show();
	};
	var onHide=function() {
	...
	return { "show":onShow,"hide":onHide }
};

EPUBJS.reader.ShareController = function() {
    var $thing = $("#shareView");
	...
	linkedinShare.onclick = function(e) {
        ...
        var linkedinwindow = window.open('http://www.linkedin.com/shareArticle?mini=true&url=' + document.URL, '', 'left=0,top=0,width=650,height=420,personalbar=0,toolbar=0,scrollbars=0,resizable=0');
