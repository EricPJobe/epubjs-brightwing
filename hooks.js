...
EPUBJS.Hooks.register('beforeChapterDisplay').swipeDetection = function(callback, renderer) {
    function detectSwipe() {
    	var book = this.book;
        var script = renderer.doc.createElement('script');
        script.text = /*"var hammertime = new Hammer.Manager(document, {\n" +
            "        touchAction: 'auto',\n" +
            "    });\n" +
            "\n" +
            ...
            "          parent.reader.book.nextChapter();\n" +
            "      });\n" +
            "      hammertime.on('swiperight', function() {\n" +
            "          parent.reader.book.prevChapter();\n" +
            "      });\n" +
    ...
    EPUBJS.core.addScript('https://cdnjs.cloudflare.com/ajax/libs/hammer.js/2.0.8/hammer.min.js', detectSwipe, renderer.doc.head);
    if (callback) {
        callback();
    }
};
